...
export class MembersComponent {
  ...
  public selectedFitlerInvitedBy: string = "";
  ...
  initData() {
    const _PROCESS = [this.getMembers(1), this.getGenderLookup()];
  ...
  onGenderLookup(gender: number) {
    const _gender: any = this.genders.filter(
      (value: any) => gender === value.id
    );
  ...
  async getGenderLookup() {
    const gender = await lastValueFrom(
      this.agendaService.genderLookup().pipe(
        map((result: any) => {
          return result;
        })
      )
  ...
  async createMember() {
    ...
    const genderFilterId = this.genders.filter(
      (gender: any) => gender.id === memberValue.gender.id
    );
    const body = {
  ...
  async globalSearchMemberFilter() {
    ...
    const members = await lastValueFrom(
      this.agendaService
        .getMembersByPage(1, { name: this.searchMemberGlobal })
        .pipe(
          map((result: IUserProps) => {
            return result;
          })
  ...
  async onFilterInvitedBy(event: any) {
    if (event.originalEvent.type === "input") {
      const value = event.value;
      const members = await lastValueFrom(
        this.agendaService
          .getMembersByPage(1, { name: value.toUpperCase() })
          .pipe(
            map((result: IUserProps) => {
              return result;
            })
      ...
      this.filterInvitedBy.set({ ...members });
      if (members) {
        this.pager.set({
          page: 1,
          totalRows: members.pager.totalItems,
      ...
      this.userFormGroup.patchValue({
        invitedByMemberName: event.value.name,
      });
    }
  }
  ...
  async deleteMembers() {
    ...
    if (deleteMembers) {
      ...
      this.defaultFilterRows =
        this.defaultFilterRows - this.selectedMembers.length;
  ...
  showEditMemberModal(member: any) {
    ...
    const filterGender = GENDER_SELECTION.filter(
      (gender: any) => gender.id === member.gender
    );
  ...
  async submitUpdateMember() {
    ...
    const body = {
      ...
      birthDate: moment(this.userFormGroup.value.birthDate).format(
        "MM/DD/YYYY"
      ),
    ...
    const updateMember = await this.updateMember(body).then(() => {
      this.messageService.add({
        key: "member",
        severity: "info",
